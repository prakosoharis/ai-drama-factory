...
class ValidationTest(unittest.TestCase):
    ...
    def test_valid_shot_11_history(self):
    ...
    def test_duplicate_candidate_id(self):
        ...
        source = project / "project/shots/shot-011/candidates/cand-shot-011-001/candidate.json"
        duplicate = project / "project/shots/shot-011/candidates/cand-shot-011-002/duplicate.json"
        duplicate.write_text(source.read_text())
        self.assertTrue(self.findings(project))

    def test_duplicate_candidate_version(self):
        temp, project = self.copied_project(); self.addCleanup(temp.cleanup)
        candidate = project / "project/shots/shot-011/candidates/cand-shot-011-002/candidate.json"
    ...
    def test_rejected_candidate_selected_and_missing_cut_reference(self):
        temp, project = self.copied_project(); self.addCleanup(temp.cleanup)
        selection = project / "project/shots/shot-011/selections/sel-shot-011-001.json"
        data = json.loads(selection.read_text()); data["candidate_id"] = "cand-shot-011-001"; selection.write_text(json.dumps(data))
        cut = project / "project/cuts/rough-cut/cut-rough-002.json"
        data = json.loads(cut.read_text()); data["timeline"][0]["candidate_id"] = "cand-shot-011-999"; cut.write_text(json.dumps(data))
    ...
    def test_checksum_mismatch_and_failed_job(self):
        temp, project = self.copied_project(); self.addCleanup(temp.cleanup)
        media = project / "project/shots/shot-011/candidates/cand-shot-011-002/video.placeholder"
        ...
        job = project / "project/shots/shot-011/jobs/job-shot-011-002.json"
